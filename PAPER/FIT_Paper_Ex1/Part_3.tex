...
Concerning the plutonium vector used to build the MOX fuel, a large range has been imposed on plutonium isotopes. Indeed, the impact of a \gls{FLM} in relation to \gls{FF} approach on a fuel cycle calculation will increase with the plutonium fraction deviation from a reference case. Perfectly well tuned scenario for which plutonium stocks are well known and approximately constant with time can be properly solved with a \gls{FF} approach since the \gld{FLM} would produce similar results. A wide range of scenario applications is considered here. From the plutonium vector produced from a small burn-up discharge CANDU to a multi-reprocessed plutonium in PWR 
